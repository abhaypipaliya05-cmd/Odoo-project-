...
export default function TripDetailPage() {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      setTrip({ ...trip, visibility: 'public', shareToken: res.shareToken });
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl p-10 border border-slate-200 shadow-sm text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 font-medium">Loading itinerary details...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="absolute bottom-6 left-4 right-4 sm:left-8 sm:right-8 text-white z-10 max-w-4xl space-y-3">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-200">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span>Target Budget: {formatCurrency(trip.budget, trip.currency)}</span>
            </div>
          </div>
        </div>
      </div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
                      ...
                      <div className="flex items-center gap-4 text-xs font-semibold text-slate-700">
                        <div className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl">
                          Stay: {formatCurrency(stop.accommodationCost, trip.currency)}
                        </div>
                        <div className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl">
                          Transit: {formatCurrency(stop.transportCost, trip.currency)}
                        </div>
                      </div>
                    </div>

                    {stop.notes && (
                      <p className="text-xs text-slate-600 bg-slate-50 p-3.5 rounded-xl border border-slate-100 italic">
                        💡 {stop.notes}
                      </p>
                    )}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div className="p-3.5 rounded-xl border border-slate-100 bg-slate-50/50 flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <p className="text-xs font-bold text-slate-800">City Exploration & Landmarks</p>
                            <p className="text-[11px] text-slate-500">Guided walking tour & photography</p>
                          </div>
                          <span className="text-xs font-semibold text-slate-700">$45</span>
                        </div>

                        <div className="p-3.5 rounded-xl border border-slate-100 bg-slate-50/50 flex items-start gap-3">
                          <Clock className="w-4 h-4 text-sky-500 mt-0.5" />
                          <div className="flex-1">
                            <p className="text-xs font-bold text-slate-800">Local Gastronomy & Dinner</p>
                            <p className="text-[11px] text-slate-500">Evening food tasting in historic quarter</p>
                          </div>
              ...
              <div className="bg-white rounded-3xl border border-slate-200 p-10 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                    Start adding cities, arrival dates, and activities to complete your itinerary.
                  </p>
                </div>
              </div>
            )}
              ...
              {trip.stops?.map((stop, idx) => (
                <div key={stop.id} className="py-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-slate-800">{stop.cityName}, {stop.country}</p>
                      <p className="text-xs text-slate-500">
          ...
          <div className="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-sm space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">Estimated expenses vs target allocation</p>
              </div>
              <div className="text-right">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-slate-100">
              <div className="p-4 rounded-2xl bg-sky-50/60 border border-sky-100">
                ...
                <span className="text-lg font-bold text-sky-950 mt-1 block">
                  {formatCurrency(
                    trip.stops?.reduce((acc, s) => acc + (s.accommodationCost || 0), 0) || 0,
                    trip.currency
                  )}
                </span>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100">
                ...
                <span className="text-lg font-bold text-indigo-950 mt-1 block">
                  {formatCurrency(
                    trip.stops?.reduce((acc, s) => acc + (s.transportCost || 0), 0) || 0,
                    trip.currency
                  )}
                </span>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-emerald-50/60 border border-emerald-100">
                ...
                <span className="text-lg font-bold text-emerald-950 mt-1 block">
                  {formatCurrency(450, trip.currency)}
                </span>
              </div>
            </div>
          </div>
