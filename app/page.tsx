...
import { DashboardStats, Destination } from '@/types';
...
export default function DashboardPage() {
  ...
  const handleToggleSave = async (destinationId: string) => {
    try {
      const res = await api.toggleSaveDestination(destinationId);
      if (stats) {
        setStats({
          ...stats,
          recommendedDestinations: stats.recommendedDestinations.map((d) =>
            d.id === destinationId ? { ...d, isSaved: res.isSaved } : d
          ),
          savedDestinationsCount: res.isSaved
            ? stats.savedDestinationsCount + 1
            : Math.max(0, stats.savedDestinationsCount - 1),
        });
      }
    } catch (err) {
        ...
        {error && (
          <div className="bg-rose-50 border border-rose-200 rounded-2xl p-6 text-rose-800 flex items-start gap-4 mb-8">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-sm">Failed to load dashboard data</h3>
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              ...
              >
                ...
                <div className="mt-4">
                  <div className="text-2xl sm:text-3xl font-bold text-slate-900">{stats.totalTrips}</div>
                  <p className="text-xs text-slate-500 mt-1 flex items-center gap-1">
                    <span>{stats.upcomingTrips} active / upcoming</span>
                  </p>
                </div>
              </Link>
              ...
              >
                ...
                <div className="mt-4">
                  <div className="text-2xl sm:text-3xl font-bold text-slate-900">{stats.upcomingTrips}</div>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Popular global hubs with realistic cost indexes and highlights</p>
                </div>
                <Link
                  href="/explore"
                  className="inline-flex items-center gap-1 text-xs font-semibold text-sky-600 hover:text-sky-700 group"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.recommendedDestinations.map((dest) => (
                  <div
                    key={dest.id}
                  ...
                  >
                    <div className="relative h-44 bg-slate-100 overflow-hidden">
                      ...
                      <button
                        ...
                        title={dest.isSaved ? 'Remove from wishlist' : 'Save to wishlist'}
                      >
                        <Bookmark
                          className={`w-4 h-4 ${dest.isSaved ? 'text-rose-500 fill-rose-500' : 'text-white'}`}
                        />
                      </button>
                      ...
                      <div className="absolute bottom-3 left-3 right-3 text-white">
                        ...
                        <p className="text-[11px] text-slate-200">{dest.country} • {dest.region}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-4 flex flex-col flex-grow justify-between gap-3">
                      ...
                      <div className="flex items-center justify-between text-xs border-t border-slate-100 pt-3">
                        <div>
                          ...
                          <span className="font-bold text-slate-900">${dest.avgDailyCost}/day</span>
                        </div>
