...
export function TripCard({ trip, onDelete, onClone, onShare }: TripCardProps) {
  ...
  const statusConfig: Record<string, { bg: string; dot: string; label: string }> = {
    PLANNED: { bg: 'bg-sky-50 text-sky-700 border-sky-200', dot: 'bg-sky-500', label: 'Planned' },
    IN_PROGRESS: { bg: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500 animate-ping', label: 'Active Now' },
  ...
  const currentStatus = statusConfig[trip.status] || statusConfig.PLANNED;
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {trip.cities && trip.cities.length > 0 ? (
              trip.cities.slice(0, 3).map((cityName, idx) => (
                <span
                  key={idx}
              ...
              <span className="text-[11px] text-slate-500">
                {trip.destinationsCount || 1} destination stop{(trip.destinationsCount || 1) > 1 ? 's' : ''}
              </span>
            )}
            {trip.cities && trip.cities.length > 3 && (
